...
function loadData(jsonArray) {
    ...
    for (var i = 0; i < objs.length; i++) { //循环后台传过来的Json数组
        ...
        var trs = '<tr>  '
            ...
            + ' <td class="hidden-480" ><textarea name="publicKey">' + publicKey + '</textarea></td> '
            + ' <td class="hidden-480"><textarea name="secretKey">' + secretKey + '</textarea></td>'
            + ' <td class="hidden-480" ><textarea name="modules">' + modules + '</textarea></td> '
            + ' <th class="hidden-480" onclick="saveOrUpdateRsa(this.parentNode)" ><button>' + "确定修改" + '</button></th> '
            + ' </tr> ';
    ...
    $(obj).children('td').each(function (j) { //遍历 tr 的各个 td
        ...
        switch (this.childNodes[0].name) {
            case "url":
                ...
                break;
            case "publicKey":
                publicKey = this.childNodes[0].value;
                break;
            case "secretKey":
                secretKey = this.childNodes[0].value;
                break;
            case "modules":
                modules = this.childNodes[0].value;
                break;
        }
    });
    var send = {
