 ...
 */
public class RequestDecodeAw {

    public static byte[] sAesKey = null;



   public static JSON decodeDataAw(String host_origin, String requestUrl, Map<String, String> headers, String httpBody, JSON dynamicSsecretKey){
        if(!StringUtils.isBlank(requestUrl) && !StringUtils.isBlank(httpBody)){
            try{
                if(requestUrl.endsWith("awt")|| requestUrl.endsWith("apl") || requestUrl.endsWith("awl")){
...
//                                        bytes = ZipUtils.ungzip(bytes);
    ...
    public static JSON decodeAw(String json) throws Exception {
